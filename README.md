...
- [数据库期末大作业-Online bookstore](#数据库期末大作业-online-bookstore)
  ...
  * [三、其他](#三其他)
    ...
    + [3.3 Git](#33-git)
    + [3.4 持续集成](#34-持续集成)
    + [3.5 打包 Docker Image](#35-打包-docker-image)
  * [四、分工](#四分工)
...
- 用户评论：多对多关系，一位买家可以对多家商店下的多本书进行评论，一家商店下的一册书也可以由多位买家评论
...
- 实现：判断 `user_id` 、`store_id` 、`book_id` 和 `order_id` 是否存在，即判断该用户是否在该商店下购买过该本书籍，存在的话判断用户是否已经对该商品进行了评论，如果已经进行了评论则返回错误，否则将用户的评论添加到评论表中。
- 分析：涉及五次查询操作和一次插入操作。
...
- 实现：判断 `user_id` 、`store_id` 和 `book_id` 是否存在，存在的话查询该本书下的评论。
- 分析：涉及四次查询操作。
...
- 实现：判断 `user_id` 、`store_id` 是否存在，存在的话查看用户购物车中是否已经存在该本书籍，存在的话更新书籍数量，不存在的话添加到购物车中。
- 分析：涉及三次查询操作及一次更新或插入操作。



#### f20. 查看购物车*

- 实现：判断 `user_id` 是否存在，存在的话查询 `user_cart` 中 `user_id` 的购物车情况。
- 分析：涉及两次查询操作。
...
- 实现：判断 `user_id` 是否存在，存在的话从 `new_order` 表中查询历史订单。
- 分析：涉及两次查询操作。
...
持续集成（Continuous Integration, CI）指代码仓库一旦进行了变更，就会按照制定的脚本或文件，自动构建并测试项目代码。如果运行结果正常，则将新代码集成到主干。

使用持续集成的好处在于，对于每次的变动，都会进行自动检测，保证每次更新后都可以正常运行，而不必等到开发完成后进行一次大合并，然后再解决不断出现的问题。

在这里使用 Travis-ci、Codecov 和 Coveralls 实现持续集成和自动化测试覆盖率。
